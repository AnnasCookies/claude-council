...
**Amended 2026-09-15, before execution.** The user dropped the credit line and every other
reference to the scaffold: the README attribution section, the upstream remote in CI, and the
ownership gate's upstream and attribution clauses all go. The `LICENSE` file keeps its copyright
notice because the MIT terms require it, and the ownership gate keeps asserting that. Executed in
the same sitting as PR `chore/rename-convene`; the rest of this brief is as accepted.
