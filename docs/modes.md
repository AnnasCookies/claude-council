...
| Transports | Subscription CLI first, metered API fallback recorded as `credentialFallback`; billing modes `sub-first`, `sub-only`, `api-only` | A per-session spend cap. Fast modes pin `sub-only`. A handler mode is handed a spend ledger the CLI built from `--spend-cap` or the mode's own default                   |
| Execution  | One runner: blind round, rebuttal round, optional refinement                                                                     | Three patterns: **streaming** (advisor), **parallel** (one blind round), **rounds** (rebuttal). The existing runner is the third. An envelope carries at most six rounds |
