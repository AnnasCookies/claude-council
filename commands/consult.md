...
5. Exit `2` is a usage error to correct, `3` a policy block to report. Exit `4` is a completed but degraded run: read `degraded` for why before deciding what to do about it. An exit `4` whose `degraded` holds only `context-skipped` or `context-truncated` entries is a complete run with omissions — say which files were left out and let the reader decide whether to re-run with narrower `--context`, rather than treating it as a failure to retry.
6. Present each report under its lens and seat, then the conflicts exactly as returned. The conflicts are never resolved by the engine and must not be resolved in your summary either: say which lenses disagree and what each holds, and leave the decision to the reader. Name any consultant that did not report, and say when the session spend cap stopped a follow-up.
