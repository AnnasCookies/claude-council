...
- Invoke only `bun --no-install ${CLAUDE_PLUGIN_ROOT}/dist/cli.js <command> ...`. Always pass `--caller human --harness claude-code`. Never invoke a legacy execution script or provider CLI directly, source TypeScript or install packages.
